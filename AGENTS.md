...
- 2026-03-18 Landing CTA dedupe (follow-up): removed only the redundant hero `Create Resume` CTA in `src/pages/index.astro` so the hero keeps a single `/app/resumes` navigation button (`Resumes`) plus `View Templates`, with no layout/style changes and no route/component scope expansion. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-18 Full verification sweep (pre-launch) completed across public, app, print, bookmark, dev, and admin surfaces with navigation/form/preview/print/api route audit plus full repo checks. Applied low-risk canonical URL consistency fixes only: updated fallback root app URL in `src/pages/bookmarks.astro` to `https://www.ansiversa.com`, and standardized pricing CTA links to canonical `https://www.ansiversa.com/pricing` in `src/pages/app/resumes/[id].astro` and `src/pages/app/resumes/[id]/print.astro` to reduce apex/www redirect inconsistency risk. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-02 Summary AI migrated to rewrite mode only: updated Summary drawer integration in `src/pages/app/resumes/[id].astro` to use shared `<AvAiAssist mode=\"rewrite\" rewriteField=\"summary\" maxChars={RESUME_MAX.summary} />` with replace-only event wiring (`onReplaceEvent=\"av:ai-replace\"`), keeping save flow unchanged and leaving other section AI behavior untouched. Dependency bumped to `@ansiversa/components@^0.0.167` (`package.json` + lockfile). Verification: `npm run typecheck` ✅ (0 errors, existing hint), `npm run build` ✅.
- 2026-03-02 Landing-page navigation follow-up: added a new hero CTA button `Resumes` next to `Create Resume` in `src/pages/index.astro`, linking directly to `/app/resumes` while preserving existing `View Templates` CTA layout and mobile/desktop visibility via existing `av-row-wrap` pattern. Verification: `npm run typecheck` ✅, `npm run build` ✅.
- 2026-03-01 Classic education degree visibility/contrast hotfix sync: upgraded shared dependency to `@ansiversa/components@^0.0.166` (`package.json`, `package-lock.json`) to consume Classic template degree heading explicit color fix (`text-slate-900`) and eliminate faded degree text in preview/print; Modern behavior unchanged. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Education degree visibility fix propagated to shared templates by upgrading `@ansiversa/components` to `^0.0.165` (`package.json`, `package-lock.json`), ensuring Classic/Modern/Minimal/Timeline preview/print render degree when present and avoid blank degree placeholder lines when missing. Combined with local normalizer fallback (`degree` <- `degree|title|program|name`) and local Modern override conditional in this repo, degree now resolves/render consistently across edit preview iframe and `/app/resumes/[id]/print`. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Education degree render fix for preview/print pipeline: hardened data normalization in `src/modules/resume-builder/helpers.ts` to resolve degree from legacy-safe keys (`degree` fallback to `title`/`program`/`name`) in `buildResumeDataFromSections()`, and updated local Modern override template `src/modules/resume-builder/ResumeTemplateModernTwoToneLocal.astro` to render degree conditionally (`edu.degree`) so valid values appear and empty placeholders do not occupy a blank line. Shared template parity handled in components package update + version sync. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Refined social-link display precision in print header URL formatter (`src/pages/app/resumes/[id]/print.astro`): LinkedIn/GitHub now render canonical short forms (`linkedin.com/in/<slug>`, `github.com/<user>`) and only append `/...` when extra path segments exist; full URL remains intact in `href`. Generic links keep prior length-based compacting behavior (`full cleaned URL` when short, else `hostname/...`). Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Refined print-header URL display precision for contact links in `/app/resumes/[id]/print` (`src/pages/app/resumes/[id]/print.astro`): URL display now shows full cleaned host+path when short (<=40 chars, e.g., `github.com/ansiversa`, `linkedin.com/in/ansiversa`) and only falls back to `hostname/...` for long URLs. Full URL remains intact in anchor `href`; protocol/`www.` removed from display text. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Print header long-URL readability fix: updated `/app/resumes/[id]/print` contact-link formatting in `src/pages/app/resumes/[id]/print.astro` so displayed website/link text is trimmed to domain-first format (`hostname` or `hostname/...`) while preserving full URL in anchor `href`. Added shared `normalizeHref()` and URL-safe `formatDisplayUrl()` parser (protocol stripping, `www.` cleanup, path hint, fallback sanitizer) and applied to Website + links (including GitHub-normalized links). Result: cleaner print/PDF contact block with clickable full links and no long raw URL overflow dominance. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Print/PDF filename polish on `/app/resumes/[id]/print`: replaced static print title contract with sanitized dynamic filename base derived from resume basics (`FirstName-LastName-JobTitle`) in `src/pages/app/resumes/[id]/print.astro`. Added filename sanitization (`spaces -> -`, non-alphanumeric removal except hyphen, duplicate hyphen collapse, trim) with fallback to project title/`Resume` when fields are missing, and synchronized browser runtime title via inline script (`document.title = pageTitle`) for print-save dialog defaults. Explicitly no `.pdf` suffix appended in code. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Synced shared template fix for Experience company-name bold in non-Modern templates by upgrading dependency `@ansiversa/components` from `^0.0.163` to `^0.0.164` (`package.json`, `package-lock.json`). This applies Classic/Minimal/Timeline company bold hierarchy in `/app/resumes/[id]/print` and editor live preview (via `ResumeBuilderShell`) while keeping local Modern override intact. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Experience company-name hierarchy polish: updated local Modern print/preview renderer `src/modules/resume-builder/ResumeTemplateModernTwoToneLocal.astro` so Experience company line uses bold weight (`font-semibold`) while keeping role/title, date, and description styling unchanged. This aligns Modern local override with resume visual hierarchy standard and shared template updates in components. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Fixed Experience chronology contract at data-normalizer layer by adding a shared comparator in `src/modules/resume-builder/helpers.ts` (`compareExperienceChronologyDesc`) and applying it in `buildResumeDataFromSections()` on `base.experience` only. Sorting now enforces: current/present roles first, then end date descending, then start date descending (year/month numeric keys with null-safe defaults). Scope intentionally excludes template files and other sections, so editor preview iframe and `/app/resumes/[id]/print` share the same ordering across all templates (Classic/Modern/Minimal/Timeline). Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Reordered Modern template left sidebar section render sequence in `src/modules/resume-builder/ResumeTemplateModernTwoToneLocal.astro` to Ansiversa standard order `Skills -> Highlights -> Certifications -> Education -> Languages` while preserving conditional rendering (sections still render only when data exists). Moved `Links` block above the standardized section sequence to avoid interrupting required ordering. Scope is Modern renderer only (used by `/app/resumes/[id]/print`, which also powers edit-page live preview iframe); Classic/Minimal/Timeline unchanged. Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Removed section-drawer double-scroll (Experience and all section forms) by adding drawer-scoped textarea auto-grow in `src/modules/resume-builder/store.ts` (`autoGrowTextarea`, `resizeTextarea`, `scheduleDrawerTextareaResize`) and wiring the section drawer content container in `src/pages/app/resumes/[id].astro` with `@input="autoGrowTextarea($event)"` so textarea height follows content instead of trapping wheel/trackpad scroll. Hardened drawer scroll container behavior with `min-height: 0` on `src/components/ResumeEditorDrawer.astro` `.av-drawer__body` and drawer-only textarea overflow rules in `src/styles/global.css` (`overflow-y: hidden; resize: none;`). Verification: `npm run typecheck` ✅ (0 errors, existing hint only), `npm run build` ✅.
- 2026-03-01 Increased Summary section max length from 300 to 800 by updating shared limits in `src/modules/resume-builder/constraints.ts` (`RESUME_MAX.summary`), which automatically updates `/app/resumes/[id]` textarea `maxlength` + live counter and server-side sanitization/validation via `RESUME_MAX.summary` in `src/actions/resumeBuilder.ts`. DB safety check: resume item payload is stored in `ResumeItem.data` as `column.text()` in `db/tables.ts`, so no migration required. Verification: `npm run typecheck` ✅, `npm run build` ✅.
